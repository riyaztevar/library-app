...
sudo dnf install -y awscli python-pip >> $LOG_FILE
...
pip3 install boto3 botocore ansible >> $LOG_FILE 2>&1
export PATH=$PATH:/home/ec2-user/.local/bin >> ~/.bashrc
ansible-galaxy collection install community.general ansible.posix amazon.aws >> $LOG_FILE 2>&1
